...
@section('preload')
    <div class="offcanvas offcanvas-end" tabindex="-1" id="offcanvasCategory" aria-labelledby="category">
        <div class="offcanvas-header">
            <h5 class="offcanvas-title" id="category">{{ __('Thêm mới chuyên mục') }}</h5>
        ...
        <div class="offcanvas-body">
            <x-admin.category.form :categories="$categories">
                <x-slot:after>
                    ...
                    <input type="hidden" name="redirect" value="{{ route('admin.post.create') }}" />
                </x-slot:after>
            </x-admin.category.form>
        </div>
    </div>
    <div class="offcanvas offcanvas-end" tabindex="-1" id="offcanvasTags" aria-labelledby="tags">
        <div class="offcanvas-header">
            <h5 class="offcanvas-title" id="tags">{{ __('Thêm mới thẻ') }}</h5>
            <button type="button" class="btn-close" data-bs-dismiss="offcanvas" aria-label="Close"></button>
        </div>
        <div class="offcanvas-body">
...
@section('content')
    ...
    <div class="container-fluid">
        <form action="{{ route('admin.post.store') }}" method="POST" accept-charset="utf-8" enctype="multipart/form-data">
            @csrf
            ...
            <div class="row">
                ...
                <div class="col-3 pt-3">
                    <div id="sticky-box">
                        <div class="card">
                            <div class="card-header fw-bold d-flex justify-content-between" type="button" data-bs-toggle="collapse" data-bs-target="#collapseCardShow" aria-expanded="false" aria-controls="collapseCardBody">
                                    {{ __('Hiển thị') }}
                                    <i class="collapseIcon bi bi-chevron-down"></i>
                            </div>
                            <div id="collapseCardShow" class="collapse">
                                <div class="card-body">
                                        ...
                                        <div class="flex-grow-1">
                                            <select id="status" name="status" class="form-select form-select-sm" aria-label="Small select example">
                                                @foreach ($status as $value => $label)
                                                    <option value="{{ $value }}" @if (old('status') == $value) selected @endif>{{ __($label) }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
                                    <div class="mb-3">
                                        <div class="form-check form-switch">
                                            ...
                                            <input class="form-check-input" type="checkbox" role="switch" id="sticky" name="sticky" value="1" @if (old('sticky')) checked @endif>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="card mt-3">
                            <div class="card-header fw-bold d-flex justify-content-between" type="button" data-bs-toggle="collapse" data-bs-target="#collapseCategoryTag" aria-expanded="false" aria-controls="collapseCardBody">
                                {{ __('Chuyên mục & Thẻ') }}
                                <i class="collapseIcon bi bi-chevron-down"></i>
                            </div>
                            <div  id="collapseCategoryTag" class="collapse">
                                <div class="card-body">
                                <div class="mb-3 row g-1">
                                    <div class="form-label col-4">{{ __('Chuyên mục') }}:</div>
                                    <div class="col-8">
                                        <div class="input-group input-group-sm">
                                            <select name="category_id" class="form-select form-select-sm select2"
                                                aria-label="Small select example" required>
                                                @foreach ($categories as $item)
                                                    <x-admin.category.option :category="$item" />
                                                @endforeach
                                            </select>
                                            <div class="input-group-text">
                                                <a class="" data-bs-toggle="offcanvas" href="#offcanvasCategory"
                                ...
                                <div class="mb-3 row">
                                    <div class="form-label col-2">{{ __('Thẻ') }}:</div>
                                    <div class="col-10">
                                        <div class="input-group input-group-sm">
                                            <select name="tags[]" multiple class="form-select form-select-sm select2"
                                                aria-label="Small select example">
                                                @foreach ($tags as $item)
                                                    <x-admin.category.option :tag="$item" />
                                                @endforeach
                                            </select>
                                            <div class="input-group-text">
                                                <a class="" data-bs-toggle="offcanvas" href="#offcanvasTags"
                                                    role="button" aria-controls="offcanvasTags">
                                                    <i class="bi bi-plus-circle-fill"></i>
                                                </a>
                                            </div>
                                        </div>
                        ...
                        <div class="card mt-3">
                            <div class="card-header fw-bold d-flex justify-content-between" type="button" data-bs-toggle="collapse" data-bs-target="#collapseImage" aria-expanded="false" aria-controls="collapseCardBody">
                                {{ __('Ảnh đại diện') }}
                                <i class="collapseIcon bi bi-chevron-down"></i>
                            </div>
