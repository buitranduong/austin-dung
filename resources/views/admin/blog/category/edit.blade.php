...
@section('preload')
    <x-admin.offcanvas title="{{ __('Seo Meta') }}" scroll="true" backdrop="false" id="offcanvasSeo">
        <x-admin.seo-meta :meta_data="$meta_data" :related_content='$category->related_content' :head_script_before="$category->head_script_before" :head_script_after="$category->head_script_after" :body_script_before="$category->body_script_before" :body_script_after="$category->body_script_after" action="{{ route('admin.category.seo-meta',[$category->id]) }}">
            <x-slot:after>
    ...
    <div class="container-fluid">
        <form action="{{ route('admin.category.update', [$category]) }}" method="POST" autocomplete="off"
            ...
            <div class="d-flex mb-3 pb-3 border-bottom">
                ...
                <div>
                    @can('CategoryController.seoMeta')
            ...
            <div>
                <div class="mb-3">
                    <label for="name" class="form-label fw-bold">{{ __('Tên danh mục') }}</label>
                    <input type="text" class="form-control" id="name" name="name" value="{{ $category->name }}"
                        aria-describedby="nameHelp" required  @cannot('CategoryController.edit') disabled @endif>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" id="slug" name="slug" value="{{ $category->slug }}"
                        aria-describedby="slugHelp" required @cannot('CategoryController.edit') disabled @endif>
                ...
                <div class="mb-3">
                    <div class="form-check form-switch">
                        <input @if($category->featured) checked @endif class="form-check-input" name="featured" value="1" type="checkbox" role="switch" id="flexSwitchCheckDefault" aria-describedby="featuredHelp" @cannot('CategoryController.edit') disabled @endif>
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control mini-editor" name="description" @cannot('CategoryController.edit') disabled @endif>{{ $category->description }}</textarea>
                </div>
                <div class="card mt-3">
                        ...
                        <div class="mb-3">
                            <div class="input-group custom-file-button">
                                ...
                                <input name="featured_image" accept=".jpg,.jpeg,.png,.webp,.gif" class="form-control"
                                    type="file" id="featured_image" @cannot('CategoryController.edit') disabled @endif>
                ...
                <div class="mt-3 d-none">
                    <label for="parent" class="form-label fw-bold">{{ __('Danh mục cha') }}</label>
                    <select name="parent_id" id="parent" class="form-select" aria-label="Default select example">
                        @if ($category->parent_id)
                            <option value="">{{ __('--- thư mục gốc ---') }}</option>
                        @else
    ...
    <script type="text/javascript">
        document.addEventListener('DOMContentLoaded', function() {
            tinymce.init({
                ...
                entity_encoding : "raw",
                statusbar: false,
                readonly: {{ Auth::user()->can('CategoryController.edit') ? 'false' : 'true' }}
            });
