...
@section('content')
    <div class="container-fluid">
        <form action="{{ route('admin.seo.file.store') }}" method="POST" autocomplete="off">
            @csrf
            ...
            <div>
                <div class="mb-3">
                    ...
                    <div id="nameHelp" class="form-text fst-italic text-danger-emphasis">Lưu ý: {{ __('Tên file') }}
                        không được dài
                        quá 255 ký tự
                    </div>
                    @error('name')
                ...
                <div class="mb-3">
                    ...
                    <div id="code-editor" class="w-100" style="height: calc(100vh - 360px)">{{ old('content') }}</div>
                    @error('content')
                    <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
    ...
    <script type="text/javascript">
        $(function() {
            ...
            $('input[name="name"]').change(function (){
                const str = $(this).val();
                const mode = str.slice(str.lastIndexOf(".")).replace('.', '');
